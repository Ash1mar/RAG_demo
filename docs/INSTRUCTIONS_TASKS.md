...
1. **????? heuristics**???????????????????? intent ???????????? / due_range / multi-person filter ???????????????????????????????
2. **?????? LLM / ???**??? `answer_mode=task_count_by_status` ?????????? LLM ???????????????????? parser ???????????????????????????? `answer_mode=default`?
3. **????? IR / SQL ??**??????????? `TaskQuerySpec`?`build_task_query_plan`?answer formatter ??????????? `_post_process_intent` ?????????? heuristics?
