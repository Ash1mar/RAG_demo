...
- Chinese embeddings (mock / remote bge-small-zh container via HTTP service).
- Task status Q&A API (`/tasks/ask`) with multiple resolver modes (rules, embeddings, hybrid, hybrid_plus_rules, hybrid_llm, text2sql) backed by SQLite or SQL Server.
...
RAG_demo/
  app/
    ...
    services/
      ...
      embeddings.py           # Embedder: mock / remote HTTP (requires EMB_URL)
      ...
      kg_lite.py              # KG-lite: persons/projects/categories/status/priority dictionary
    ...
    tasks_store/
      ...
      mssql_store.py          # MSSQLTasksStore (SQL Server, read-only)
  ...
  data/
    ...
    kg_data.json              # KG-lite data: persons/projects/categories/tags/statuses/priorities
  embedder_server/
  ...
  scripts/
    ...
    extract_kg_from_tasks.py  # Preview canonical KG-lite candidates (persons/projects/categories/statuses/priorities)
  ...
  config/
    app.env                   # Centralized env config (optional)
...
- **NL->JSON->SQL experiment (`/db/ask`)**
  - A pure NL->IR->SQL pipeline for the `tasks`/`task_latest` tables, without natural-language answer generation:
    ...
    - `rows`: raw records returned from the tasks store after executing the query.
...
- **Text2SQL with AST validation**
  - When `resolver_mode="text2sql"` or `hybrid_llm` (Text2SQL branch enabled), `/tasks/ask` can:
    ...
    3. For each SQL: rewrite + validate via `sqlglot` AST (`app/services/task_query.py`) and then run it against `TasksStore.query`.
  - Safety checks include:
    - allow only `SELECT` statements referencing `task_latest`/`tasks` tables;
    - enforce a hard row cap (SQLite uses `LIMIT`, SQL Server uses `TOP`);
...
- **KG-lite semantic dictionary for tasks**
  ...
  - Captures domain semantics as **data**, not scattered rules:
    ...
    - categories -> tags: `"安全专项"` / `"安监专项"` map to category `"安监整改"`, which expands to tags `["整改","安全整改"]`;
    - statuses: canonical `DONE/TODO/IN_PROGRESS/BLOCKED` plus synonyms ("完成/搞定/已完成"等);
    - priorities: canonical integers (`1/2/3`) plus synonyms ("P1/高优/最高优先级"等).
  - NL->JSON: `_post_process_intent` calls `kg_lite.resolve_person/resolve_project/resolve_category_tags/resolve_status_value/resolve_priority_value` to normalize `TaskQuerySpec` fields, and records flags like `kg_enabled`, `kg_person_source`, `kg_project_source`, `kg_category_source` in `spec.extra`.
  - Text2SQL: `task_query._make_text2sql_ir_hint` includes canonical person/project/tags/status/priority in the IR hint; `_rewrite_text2sql_query` uses them to align LLM SQL output (e.g. rewriting `person = '张工'` to `person = '张三'`, `priority = '高优'` -> `priority = 1`, injecting missing `tags LIKE '%整改%'` filters).
  - Backends:
...
- **Embeddings**
  - `MOCK_EMB=1|0`: enable mock embeddings (1) or remote service (0).
  - `EMB_URL`: remote embedding HTTP endpoint (required when `MOCK_EMB=0`), e.g. `http://localhost:8080/embeddings`.
  - `EMB_DIM`: embedding dimension (depends on the remote model; bge-small-zh=512).
  - `EMB_TIMEOUT`: HTTP timeout (seconds, default 12).
  - `MODEL_NAME`: optional hint passed to external embedder services (kept for compatibility, unused by the API's mock/HTTP client).
...
- **Centralized config (optional)**
  - `APP_CONFIG=config/app.env`: load an env file at startup (CLI env vars still override).
- **LLM / NL2SQL / Text2SQL**
...
- **Entry points & modes**
  - `/tasks/ask` when `resolver_mode="text2sql"` or `resolver_mode="hybrid_llm"`:
    ...
    4. For each query, apply `_rewrite_text2sql_query` + AST validation, then execute with `TasksStore.query`.
...
- **Safety and rewriting (`app/services/task_query.py`)**
  - Use `sqlglot` to parse SQL into AST:
    ...
    - inject or cap a row limit (SQLite uses `LIMIT`, SQL Server uses `TOP`) and normalize `ORDER BY` placement;
